...
class DCMTrajectoryGenerator:
    def __init__(self,pelvisHeight,  stepTiming,  doubleSupportTime):
        ...
        self.timeStep = 1/240 #We select this value for the timestep(dt) for discretization of the trajectory.
        # The 240 Hz is the default numerical solving frequency of the pybullet. Therefore we select this value for DCM trajectory generation discretization.
    ...
    def embedDoubleSupportToDCMTrajectory(self): #Calculate and replace DCM position for double support with the corresponding time window of preliminary single support phase
        ...
        for stepNumber in range(np.size(self.CoP,0)):
            if(stepNumber==0):
                ...
                doubleSupportInterpolationCoefficients.append(
                    DCMTrajectoryGenerator.doInterpolationForDoubleSupport(self, 
                                                                           ...
                                                                           (1-self.alpha)*self.dsTime))
                #CreterpolationForDate a vector of DCM Coeffient by using the doInterpolationForDoubleSupport function. Note that the double support duration for first step is not the same as other steps
        ...
        for stepNumber in range(np.size(self.CoP,0)):
            ...
            if(stepNumber==0):#notice double support duration is not the same as other steps
                ...
                for t in range(int((1-self.alpha)*self.dsTime*(1/self.timeStep))):
                    ct = t*self.timeStep
                    doubleSupportTrajectory[t] = (ct**3)*a + (ct**2)*b + ct*c + d #use equation 16 (only the DCM position component)
        ...
        DCMCompleteTrajectory = np.array(self.DCM)#First we put preliminary DCM trajectory into a new array and in the following we will replace the double support part
        ...
        for stepNumber in range(self.CoP.shape[0]):
            if stepNumber == 0:
                ...
                start = 0
                end = int(self.numberOfSamplesPerSecond*(1-self.alpha)*self.dsTime)
                #fill the corresponding interval for DCM index for double support part
            else: 
                start = int(self.numberOfSamplesPerSecond*stepNumber*self.stepDuration - self.numberOfSamplesPerSecond*self.alpha*self.dsTime)
                end = int(self.numberOfSamplesPerSecond*stepNumber*self.stepDuration + self.numberOfSamplesPerSecond*(1-self.alpha)*self.dsTime)
